...
var distUtils = require('../utils/dist-utils');
...
module.exports = function (grunt) {
  ...
  //
  // 该 Grunt 任务是截取 "/** SOURCE-CODE-START */" 与
  ...
  grunt.registerTask('dist', function () {
    // 获取来源文件路径和目标文件路径
    var sourceFilePaths = grunt.config('dist.src');
    var destFilePath = grunt.config('dist.dest');
    ...
    for (var index = 0; index < sourceFilePaths.length; index++) {
      ...
      // 计算源代码的开始和结束索引
      var startIndex = distUtils.getStartSourceIndex(content);
      var endIndex = distUtils.getEndSourceIndex(content);
    ...
    var destFileContent = SOURCE_TEMPLATE.replace(/\{\{version\}\}/, 'v'.concat(pkgVersion));
